...
/**
 * Custom files added for logging
 */
...
class AMFBaseDeserializer {
	...
	function AMFBaseDeserializer($rd) {
		...
		$this->logger =& LoggerManager::getLogger('AMFBaseDeserializer');
	...
	function readHeader() {
		...
		$this->logger->info('AMF Header: '.$topByte. " : This value should be 0 or 3");
		...
		while ($this->header_count--) { // loop over all of the header elements
			...
			$this->logger->info('AMF Header Content: '.$name.' : '.$content);
